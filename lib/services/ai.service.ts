...
const stripAiInvocation = (content: string) => content.replace(/@ai\b/gi, "").trim()

const formatHumanMessage = (name: string | null | undefined, content: string) => {
  const label = name?.trim() || "User"

  return `${label}: ${content}`
}

const buildPromptMessages = async (roomId: string, triggerMessageId: string): Promise<AiPromptMessage[]> => {
  const contextMessages = await messageRepository.listRecentByRoomId(roomId, ROOM_CONTEXT_LIMIT)
  const triggerMessage = contextMessages.find((message) => message.id === triggerMessageId)
...
export const aiService = {
  shouldInvokeAssistant: (content: string) => AI_INVOCATION_PATTERN.test(content),
  ...
  streamAssistantReply: async ({
    ...
    let resolveMessage: ((value: Awaited<ReturnType<typeof messageRepository.create>>) => void) | undefined
    let rejectMessage: ((reason?: unknown) => void) | undefined

    const persistedMessage = new Promise<Awaited<ReturnType<typeof messageRepository.create>>>(
      (resolve, reject) => {
